...
def productivity_index(request, pk):
    ...
    if request.method == 'POST':
        ...
        if request.is_ajax():
            ...
            base_url = reverse('productivity_index', args=[pk])
...
def tasks_tracking(request, tasks_tracking, pk):
    user_id = User.objects.get(pk=pk)
    if request.is_ajax():
        ...
        if 'remove_task' in request.POST:
            ...
            base_url = reverse('tasks_tracking', args=[tasks_tracking, pk])
            query_string = urlencode(date_contain)
            url = '{}?{}'.format(base_url, query_string)
            return HttpResponseRedirect(url)
